...
class Sprint5SaadStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        dynamo_db = self.create_table("Saad_tableV1", "Timestamp","Subject")
        ...
        WHLambda = self.create_lambda("WebHealthLambdaV1", "./resources", "WebHealthLambda.lambda_handler", role )     # creation of lambda function with role and removal policy
        ...
        dynamodb_lambda.add_environment('tableNameV1', tableName)

        my_topic = sns_.Topic(self, "SaadHassanTopicV1")                     # creating an sns topic and then adding an email subscription to the sns topic
        ...
        webCrawler = self.create_table('Saad_webCrawlerV1', 'id_', 'url')
        ...
        bktToDBHandler.add_environment('bucketNameV1', bucketName)
        bktToDBHandler.add_environment('S3TotableV1', webCrawler.table_name)
        WHLambda.add_environment('bucketNameV1', bucketName)
        WHLambda.add_environment('S3TotableV1', webCrawler.table_name)
        WHLambda.add_environment('topic_arnV1', my_topicARN)
        ...
        ApiHandler = self.create_lambda("SaadApiHandlerV1", "./resources", "api_LambdaHandler.api_Handler", role)  
        ApiHandler.apply_removal_policy(RemovalPolicy.DESTROY)
        ApiHandler.add_environment('bucketNameV1', bucketName)
        ApiHandler.add_environment('S3TotableV1', webCrawler.table_name)
        webCrawler.grant_full_access(ApiHandler)


        # instantiating API Gateway
        self.create_api('Saad-CRUD-Api-V1', ApiHandler)
